...
const STEPS = [
  {
    speech: "Welcome to the HE Library Benchmark!",
    body:   "I'll guide you through how to read these results like a cryptography researcher. We ran identical operations across three libraries — same security level, same ring size — so every difference you see is due to the library itself, not the parameters.",
    table:  null,
  },
  {
    speech: "Three libraries, three different design goals",
    body:   "SEAL (Microsoft) prioritises correctness and portability. HELib (IBM) was the first practical FHE library and optimises for BGV depth. OpenFHE is the successor to PALISADE — built for production performance with a modern modular API.",
    table: {
      headers: ["Library", "Origin", "Scheme", "Primary Goal"],
      rows: [
        ["SEAL",    "Microsoft Research", "BFV", "Correctness & ease of use"],
        ["HELib",   "IBM Research",       "BGV", "Depth & bootstrapping"],
        ["OpenFHE", "DARPA / Community",  "BFV", "Production throughput"],
      ],
    },
  },
  {
    speech: "BFV vs BGV — why the scheme matters",
    body:   "BFV (SEAL & OpenFHE) embeds the message in the high bits scaled by Δ. BGV (HELib) stores it in the low bits and uses modulus switching to drain noise after each multiplication. BGV is theoretically superior for deep circuits, but BFV is simpler to implement and tune — which is why SEAL and OpenFHE both chose it.",
    table: {
      headers: ["Property",     "BFV (SEAL / OpenFHE)",    "BGV (HELib)"],
      rows: [
        ["Message encoding", "Δ·m + noise (high bits)",  "m + p·noise (low bits)"],
        ["Noise management", "Scale factor Δ",            "Modulus switching"],
        ["Best for",         "Simple integer arithmetic", "Deep levelled circuits"],
        ["Relin key needed", "Yes",                       "Yes"],
      ],
    },
  },
  {
    speech: "Key Generation — why SEAL is often slowest here",
    body:   "Key generation includes generating a public key, secret key, AND a relinearisation key. The relin key is a large polynomial that enables safe ciphertext multiplication. SEAL generates this key in software with no hardware acceleration, while OpenFHE uses AVX2/AVX512 SIMD intrinsics — that is why OpenFHE key-gen is typically 3–5× faster than SEAL at the same ring size.",
    table: {
      headers: ["",            "SEAL",              "HELib",         "OpenFHE"],
      rows: [
        ["Ring dim n",  "4096",              "~4096",         "4096"],
        ["Security",    "128-bit",           "~128-bit",      "128-bit"],
        ["Keys",        "Pub+Sec+Relin",     "Pub+Sec+Relin", "Pub+Sec+Relin"],
        ["SIMD accel.", "No",                "Partial",       "AVX2/AVX512"],
      ],
    },
  },
  {
    speech: "Encryption & Decryption — batching changes everything",
    body:   "SEAL and OpenFHE batch 4096 plaintext integers into a single ciphertext using NTT (Number Theoretic Transform). HELib encrypts one integer per ciphertext. So SEAL/OpenFHE encrypt 10 values in ~1 ciphertext while HELib needs 10. Their raw times look similar, but OpenFHE and SEAL are doing far more work per operation — making them dramatically more efficient in real workloads.",
    table: {
      headers: ["",           "SEAL",        "HELib",      "OpenFHE"],
      rows: [
        ["Plaintext slots", "4096",        "1",          "4096"],
        ["Ciphertexts",     "10",          "10",         "10"],
        ["Encoding",        "NTT batch",   "Single int", "NTT batch"],
        ["Effective work",  "40,960 vals", "10 vals",    "40,960 vals"],
      ],
    },
  },
  {
    speech: "Multiplication — the most expensive FHE operation",
    body:   "Ciphertext multiplication is ~100× slower than addition. After multiplying two degree-1 ciphertexts, the result is degree 2 — you must relinearise (using the relin key) to bring it back to degree 1, or noise grows exponentially. OpenFHE's SIMD-optimised NTT makes this step significantly faster than SEAL's pure C++ implementation.",
    table: {
      headers: ["",             "SEAL",            "HELib",           "OpenFHE"],
      rows: [
        ["Operation",    "ct[i]×ct[i+1]",   "ct[i]×ct[i+1]",   "ct[i]×ct[i+1]"],
        ["Post-step",    "Relinearise",      "Relinearise",      "Relinearise"],
        ["NTT backend",  "C++ only",         "Partial SIMD",     "AVX2/AVX512"],
        ["Relative cost","~Baseline",        "~Varies",          "~2–4× faster"],
      ],
    },
  },
  {
    speech: "How to read the results below",
    body:   "Lower bars = faster. The radar chart shows which library wins each operation. A library that excels at key-gen but is slow at multiply may not be the best for CNN inference. For our MNIST pipeline — which is heavy on multiplications — OpenFHE's multiply speed is what matters most.",
    table:  null,
  },
];
